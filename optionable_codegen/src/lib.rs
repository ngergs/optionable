...
use syn::{
    ...
    Generics, ImplGenerics, LitStr, Meta, MetaList, Path, PathArguments, Token, Type,
    TypeGenerics, TypePath, WhereClause, parse_quote,
};
...
const SINGLE_TYPE_PARAM_CONTAINERS: [&str; 13] = [
    "Vec", "VecDeque", "LinkedList", "BinaryHeap", "BTreeSet", "HashSet", "Option", "Box", "Arc",
    "Rc", "Cell", "RefCell", "Cow",
];
...
fn is_self_resolving_optioned(ty: &Type) -> bool {
    ...
    // Helper: iterate only over type arguments (skipping lifetimes/consts)
    ...
    if SINGLE_TYPE_PARAM_CONTAINERS.contains(&&*ident) {
        // Self-resolving if the single inner type argument is self-resolving
        type_args
            .first()
            .is_some_and(|inner| is_self_resolving_optioned(inner))
    } else if MAP_CONTAINERS.contains(&&*ident) {
        // Self-resolving if the value type (second type arg) is self-resolving
        type_args
            .get(1)
            .is_some_and(|value_ty| is_self_resolving_optioned(value_ty))
    } else if ident == "Result" {
        // Self-resolving if the Ok type (first type arg) is self-resolving
        type_args
            .first()

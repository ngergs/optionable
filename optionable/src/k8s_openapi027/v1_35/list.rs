...
impl<T> crate::Optionable for k8s_openapi027::List<T>
where
    T: k8s_openapi027::ListableResource + crate::Optionable,
    <T as crate::Optionable>::Optioned: Sized + Clone + Default + PartialEq
        + serde::de::DeserializeOwned + serde::Serialize + std::fmt::Debug,
...
impl<T> crate::OptionableConvert for k8s_openapi027::List<T>
where
    T: k8s_openapi027::ListableResource + crate::OptionableConvert,
    <T as crate::Optionable>::Optioned: Sized + Clone + Default + PartialEq
        + serde::de::DeserializeOwned + serde::Serialize + std::fmt::Debug,
{
...
#[automatically_derived]
#[cfg(feature = "k8s_openapi_convert")]
impl<T> crate::OptionedConvert<k8s_openapi027::List<T>> for ListAc<T>
where
    T: k8s_openapi027::ListableResource + crate::OptionableConvert,
    <T as crate::Optionable>::Optioned: Sized + Clone + Default + PartialEq
        + serde::de::DeserializeOwned + serde::Serialize + std::fmt::Debug,
{
    fn from_optionable(value: k8s_openapi027::List<T>) -> Self {
        crate::OptionableConvert::into_optioned(value)
    }
    fn try_into_optionable(self) -> Result<k8s_openapi027::List<T>, crate::Error> {
        crate::OptionableConvert::try_from_optioned(self)
    }
    fn merge_into(
        self,
        other: &mut k8s_openapi027::List<T>,
    ) -> Result<(), crate::Error> {
        crate::OptionableConvert::merge(other, self)
    }
}
impl<T> k8s_openapi027::Resource for ListAc<T>
where
    T: k8s_openapi027::ListableResource + crate::Optionable,
    <T as crate::Optionable>::Optioned: Sized + Clone + Default + PartialEq
        + serde::de::DeserializeOwned + serde::Serialize + std::fmt::Debug,
{
    const API_VERSION: &'static str = <k8s_openapi027::List<
        T,
    > as k8s_openapi027::Resource>::API_VERSION;
    const GROUP: &'static str = <k8s_openapi027::List<
        T,
    > as k8s_openapi027::Resource>::GROUP;
    const KIND: &'static str = <k8s_openapi027::List<
        T,
    > as k8s_openapi027::Resource>::KIND;
    const VERSION: &'static str = <k8s_openapi027::List<
        T,
    > as k8s_openapi027::Resource>::VERSION;
    const URL_PATH_SEGMENT: &'static str = <k8s_openapi027::List<
        T,
    > as k8s_openapi027::Resource>::URL_PATH_SEGMENT;
    type Scope = <k8s_openapi027::List<T> as k8s_openapi027::Resource>::Scope;
}
impl<T> k8s_openapi027::Metadata for ListAc<T>
where
    T: k8s_openapi027::ListableResource + crate::Optionable,
    <T as crate::Optionable>::Optioned: Sized + Clone + Default + PartialEq
        + serde::de::DeserializeOwned + serde::Serialize + std::fmt::Debug,
{
    type Ty = <k8s_openapi027::List<T> as k8s_openapi027::Metadata>::Ty;
...
where
    ...
    <T as crate::Optionable>::Optioned: Sized + Clone + Default + PartialEq
        + serde::de::DeserializeOwned + serde::Serialize + std::fmt::Debug
        + k8s_openapi027::Metadata<
            Ty = k8s_openapi027::apimachinery::pkg::apis::meta::v1::ObjectMeta,
        ...
        if let Some(items_other) = other.items {
            if let Some(items_self) = &mut self.items {
                for el_other in items_other {
                    if let Some(el_self) = items_self
                        .into_iter()
                        .find(|el_self| {
                            let meta_self = k8s_openapi027::Metadata::metadata(*el_self);
                            let meta_other = k8s_openapi027::Metadata::metadata(
                                &el_other,
                            );

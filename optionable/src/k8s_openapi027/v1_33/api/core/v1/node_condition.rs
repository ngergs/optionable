...
impl crate::OptionableConvert for k8s_openapi027::api::core::v1::NodeCondition {
    ...
    fn merge(&mut self, other: NodeConditionAc) -> Result<(), crate::Error> {
        if self.last_heartbeat_time.is_none() {
            self.last_heartbeat_time = other.last_heartbeat_time;
        }
        if let Some(other_value) = other.last_heartbeat_time {
            crate::OptionableConvert::merge(&mut self.last_heartbeat_time, other_value)?;
        }

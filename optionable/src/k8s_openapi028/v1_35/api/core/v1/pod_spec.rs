...
pub struct PodSpecAc {
    ...
    pub workload_ref: Option<
        <::k8s_openapi028::api::core::v1::WorkloadReference as crate::Optionable>::Optioned,
    >,
}
...
impl crate::OptionableConvert for k8s_openapi028::api::core::v1::PodSpec {
    ...
    fn merge(&mut self, other: PodSpecAc) -> Result<(), crate::Error> {
        ...
        if self.workload_ref.is_none() {
            self.workload_ref = crate::OptionableConvert::try_from_optioned(
                other.workload_ref,
            )?;
        } else if let Some(self_value) = self.workload_ref.as_mut()
            && let Some(other_value) = other.workload_ref
        {
            crate::OptionableConvert::merge(self_value, other_value)?;
        }
        Ok(())
